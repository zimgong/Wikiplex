...
ENCODED_DOCUMENT_EMBEDDINGS_NPY_DATA = DATA_PATH + \
...
class SearchEngine(BaseSearchEngine):
    def __init__(self) -> None:
        ...
        self.main_index = Indexer.create_index(
            ...
            self.stopwords, 50, doc_augment_dict=self.doc_augment_dict
        )
        self.title_index = Indexer.create_index(
            IndexType.InvertedIndex, DATASET_PATH, self.preprocessor,
            self.stopwords, 2, text_key='title'
        )

        print('Loading ranker...')
        with open(ENCODED_DOCUMENT_EMBEDDINGS_NPY_DATA, 'rb') as f:
            self.encoded_docs = np.load(f)
        self.ranker = VectorRanker(
            'sentence-transformers/msmarco-MiniLM-L12-cos-v5',
            self.encoded_docs, list(self.main_index.document_metadata.keys())
        )
        ...
        self.recognized_categories = set(
            [category for category, count in category_counts.items() if count > 1000]
        )
                ...
                while True:
                    try:
                        ...
                        if str(data['docid']) in train_docs:
                            self.raw_text_dict[str(data['docid'])] = data['text'][:500]
                    except:
                        break
            pickle.dump(
                self.raw_text_dict, 
