...
class OnDiskInvertedIndex(BasicInvertedIndex):
    def __init__(self, shelve_filename) -> None:
        ...
        if not os.path.exists(shelve_filename):
    ...
    def save(self, index_directory_name: str) -> None:
        ...
        if not os.path.exists(index_directory_name):
            os.makedirs(index_directory_name)
        file = open(index_directory_name + '/metadata.json', 'w+')
        vocabulary = list(self.vocabulary)
        json_data = {
            'statistics': self.statistics,
            'document_metadata': self.document_metadata,
            'minumum_word_frequency': self.min_word_freq,
            'shelve_filename': self.shelve_filename,
    ...
    def load(self, index_directory_name: str) -> None:
        ...
        """
        # TODO: Load the index files from disk to a Python object
        file = open(index_directory_name + '/metadata.json', 'r')
        json_data = json.load(file)
        self.statistics = json_data['statistics']
        self.document_metadata = json_data['document_metadata']
        self.min_word_freq = json_data['minumum_word_frequency']
        self.shelve_filename = json_data['shelve_filename']
